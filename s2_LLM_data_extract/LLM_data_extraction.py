...
def chat_1_step(model, messages, temperature, max_tokens, new_dir, md_dir, response_folder):
    ...
    try:
        ...
        logging.info(f"Extract result written to: {response_content_dir}")
...
def chat_2_step(md_dir, file_content, response_folder, model, temperature, new_dir, prompt_extract, max_tokens, prompt_merge_dir="prompt/p_2_0826.txt"):
    """
    ...
    """
    all_response = ""
    for i in range(len(file_content) // 120000 + 1):
        text = file_content[i * 120000:(i + 1) * 120000]
        messages = [
            {"role": "system", "content": "You are an expert in information extraction from scientific literature."},
            {"role": "user",
             "content": f"The following is a scientific article, please read it carefully: \n{text}\n{prompt_extract}"}
        ]
        tokens = num_tokens_from_messages(messages)
        logging.info(f"Step one: Extracting part {i}")
...
def LLM_extract(md_dir, file_content, response_folder, prompt_extract_dir="prompt/p_3_2_0806.txt", prompt_merge_dir="prompt/p_2_0826.txt", model="claude-3-5-sonnet-20240620", temperature=0.1,
                 max_tokens=8192):
    """
    ...
    new_dir = "prompt_" + prompt_extract_dir.split("/")[-1].split(".")[0] + "_" + model + "_128k_stream_max_tokens_" + str(
        max_tokens) + "_temperature_" + str(temperature) + "/"
    ...
    messages = [
        {"role": "system", "content": "You are an expert in information extraction from scientific literature."},
        {"role": "user", "content": f"The following is a scientific article, please read it carefully: \n{file_content}\n{prompt_extract}"}
    ]
    tokens = num_tokens_from_messages(messages)
    logging.info("Starting first round: Extraction")
    logging.info(f"Prompt tokens: {tokens}")
